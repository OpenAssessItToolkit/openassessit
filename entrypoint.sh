...
cd openassessit;
CHROME_PATH=/usr/bin/chromedriver lighthouse $1 --add-host="chrome:127.0.0.1" --only-categories=accessibility --no-enable-error-reporting --emulated-form-factor=none --chrome-flags="--headless --window-size=1300,600 --no-sandbox --disable-gpu" --output="json" --output-path="/tmp/$2/$2.json";
